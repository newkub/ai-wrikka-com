  ...
  >
    ...
    <div class="relative border-b border-border">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-3">
        <kbd class="px-2 py-1 text-xs font-semibold text-text/50 bg-surface-100 rounded">
          Esc
        </kbd>
      </div>
    </div>
    ...
    <div v-if="searchQuery" class="max-h-[60vh] overflow-y-auto">
      ...
      <ul v-else class="divide-y divide-border">
        <li 
          ...
          class="relative flex cursor-pointer select-none items-center px-4 py-3 hover:bg-surface-100 transition-colors"
          :class="{ 'bg-surface-100': selectedIndex === index }"
        >
          <div class="flex h-10 w-10 flex-none items-center justify-center rounded-lg" :class="item.icon ? 'bg-brand/10' : 'bg-surface-200'">
    ...
    <div v-else class="px-6 py-8 text-center">
      ...
      <div class="mt-4 grid grid-cols-2 gap-3 max-w-md mx-auto">
        <button
          ...
          class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-text bg-surface-100 hover:bg-surface-200 rounded-lg transition-colors"
        >

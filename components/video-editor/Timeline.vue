...
const handleDragMove = (event: MouseEvent) => {
	...
	const timePerPixel =
		props.maxDuration /
		(document.querySelector(".timeline")?.clientWidth || props.maxDuration);
...
const handleResizeMove = (event: MouseEvent) => {
	...
	if (resizeEdge === "start") {
		newStart = Math.min(
			Math.max(0, startClipStart + deltaTime),
			currentClip.end - 0.5,
		); // Min 0.5s duration
	} else {
		newEnd = Math.max(
			Math.min(props.maxDuration, startClipEnd + deltaTime),
			currentClip.start + 0.5,
		); // Min 0.5s duration
	}

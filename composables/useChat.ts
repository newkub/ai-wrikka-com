	...
	const loadSessionFromRoute = async (newId: string | string[] | undefined) => {
		try {
			if (newId && typeof newId === "string") {
				...
				const latestSession = [...chatStore.chatSessions].sort(
					(a, b) =>
	...
	const sendMessage = async (): Promise<void> => {
		...
		try {
					...
					messages: [
						{
							role: "system",
							content:
								"คุณเป็นผู้ช่วย AI ที่ช่วยตอบคำถามและให้ข้อมูลที่เป็นประโยชน์ ใช้ภาษาไทยในการตอบ",
						},
			...
			chatStore.addMessage(
				"ai",
				"ขออภัย เกิดข้อผิดพลาดในการเชื่อมต่อกับระบบ AI กรุณาลองใหม่อีกครั้ง",
			);
		} finally {

...
export function useWhiteboard(canvasRef: Ref<HTMLCanvasElement | null>) {
	...
	const startDrawing = (e: MouseEvent | TouchEvent) => {
		...
		if (activeTool.value === "text") {
			...
			if (text) {
				...
				currentDrawable.value.points.push({
					x: point.x + 100,
					y: point.y + 20,
				}); // Default size
	...
	const draw = (e: MouseEvent | TouchEvent) => {
		...
		if (["pen", "eraser"].includes(activeTool.value)) {
			...
			["line", "rectangle", "circle", "triangle"].includes(activeTool.value)
		) {
		...
		if (drawables.value.length > 0) {
			drawables.value[drawables.value.length - 1] = {
				...currentDrawable.value,
			};
		}
	...
	const stopDrawing = () => {
		...
		if (currentDrawable.value) {
			if (
				currentDrawable.value.type === "rectangle" &&
	...
	const handleImageUpload = (e: Event) => {
		...
		reader.onload = (event) => {
			if (!event || !event.target || !event.target.result || !canvasRef.value)
				return;
			...
			img.onload = () => {
				...
				drawables.value.push({
					...
					text: `data:${file.type};base64,${btoa(
						new Uint8Array(
							(event.target?.result as ArrayBuffer) || new ArrayBuffer(0),
	...
	const onKeyDown = (e: KeyboardEvent) => {
		...
		if (
			...
			drawables.value.some((d) => d.isSelected)
		) {

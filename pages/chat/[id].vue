...
function handleFileRemove(file: FileWithPreview) {
  const index = files.value.findIndex(f => f.name === file.name && f.size === file.size);
  if (index !== -1) {
...
async function sendMessage({ text, model, mode, files: messageFiles = [] }: SendMessageParams) {
...
function handleSend({ text, files: messageFiles = [] }: { text: string; files?: FileWithPreview[] }) {
